...
const logger = new Logger('AMProfileHandlers');

/**
 * Update account manager profile
 */
export async function updateAMProfile(req: AuthRequest, res: Response) {
  try {
    if (!req.user) {
    ...
    logger.info('Updating account manager profile', { userId });
    ...
    logger.info('Account manager profile updated successfully', { userId });
    ...
    logger.error('Error updating account manager profile', { error });
