...
const logger = new Logger('SuperAdminProfileHandlers');

/**
 * Update superadmin profile
 */
export async function updateSuperAdminProfile(req: AuthRequest, res: Response) {
  try {
    if (!req.user) {
    ...
    logger.info('Updating superadmin profile', { userId });
    ...
    logger.info('SuperAdmin profile updated successfully', { userId });
    ...
    logger.error('Error updating superadmin profile', { error });
